...
void Player::shoot() {
	...
	pl_bullets.add(new PlayerShot(x, y-3));
}

void Player::move() {
	// Update position on buffer matrix according to usart
	if (bitRead(*signal, UP) && y < STG_HEIGHT) ++y;
	if (bitRead(*signal, DOWN) && y > 0) --y;
...
void Player::draw() {
	// draw over pl_bullet, under enm_bullet & enm
	// tick order is crucial here
	for (int i = 0; i < 4; ++i) {
		for (int j = 0; j < 3; ++j) {
			if(ship[i][j]) matrix.drawPixel(x+j-1, y+i-1, Colors[WHITE]);
		}
	}
	matrix.drawPixel(x, y, coreColor);
}
...
void Player::anim_explode(byte frame) {
	...
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			if(explode[frame][i][j]) matrix.drawPixel(x+j-1, y+i-1, coreColor);
		}
	}
}
